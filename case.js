...
const fakey = (teks) => {
neo.sendMessage(m.chat, { text: teks, contextInfo:{"externalAdReply": {"title": `Follow me in instagram`,"body": `instagram me`, "previewType": "PHOTO","thumbnailUrl": ``,"thumbnail": fs.readFileSync(`./media/image/thumb.jpg`),"sourceUrl": "https://instagram.com/fardev78"}}}, { quoted: m})
}
const ftextt = {
key: { 
fromMe: false,
...
url: 'https://youtu.be/RMixpZRoDOk'
}
}]
neo.send5ButImg(m.chat, fitur, `IG : fardev78\nANA~BOT`, log0, btnR)
}
break
...
if (!q.includes('tiktok')) return reply('Invalid link!!')
  m.reply(mess.wait)
...
case prefix+'ytmp3':{
  ...
  if (!q.includes('youtube')) return reply('Invalid link!!')
  m.reply(mess.wait)
   try{
    await yta(args[0])
.then((res) => {
...
case prefix+'simi':
case prefix+'ana':
res = await fetchJson(`https://api.simsimi.net/v2/?text=${q}&lc=id`)
neo.sendMessage(from, { text: cimcimi.success})
break
...
neo.sendMessage(from, {caption: `Sange kok sama gambar`, image: {url: hasil}}, {quoted: msg})
break
case prefix+'blowjob':
reply(mess.wait)
...
neo.sendMessage("6283811034750@s.whatsapp.net", util.format(err), {quoted:m})
console.log(err)
}
}
